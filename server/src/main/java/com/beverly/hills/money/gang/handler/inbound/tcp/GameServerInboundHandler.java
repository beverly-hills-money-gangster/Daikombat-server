package com.beverly.hills.money.gang.handler.inbound.tcp;
        ...
        (game, playerState) -> {
          ...
          game.getPlayersRegistry().allPlayers().stream().filter(
              otherPlayerStateChannel -> otherPlayerStateChannel.getPlayerState().getMatchId()
                  ...
                  != PlayerActivityStatus.JOINING).forEach(
              playerStateChannel -> playerStateChannel.writeTCPFlush(
                  disconnectEvent, ChannelFutureListener.CLOSE_ON_FAILURE));
        });
    if (!playerWasFound) {
